...
.. Select *Cancel* to end the discovery process without verifying the certificate. For example, you can select *Cancel* if you receive an error message notifying you that the StorageGRID management certificate needs to be updated. To update a certificate, go to <<Use a different certificate>>.
